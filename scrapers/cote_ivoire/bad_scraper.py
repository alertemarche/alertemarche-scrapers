"""Robot de collecte — Côte d'Ivoire 🇨🇮 · BAD (Banque Africaine de Développement / AfDB).
...
Le paramètre d'URL `?f[0]=country:Cote dIvoire` est ignoré par le rendu HTML :
la page renvoie TOUS les avis, tous pays confondus (Éthiopie, Zimbabwe,
Namibie…), répartis sur des dizaines de pages. L'ancienne version se contentait
de chercher le mot « Ivoire » dans le texte de la page puis prenait le PREMIER
lien de la ligne — un avis d'un pays quelconque. C'est ainsi que des marchés
d'autres pays se retrouvaient dans la section Côte d'Ivoire d'AlerteMarché.

✅ Correction : chaque avis AfDB encode son pays de façon fiable dans l'URL ET
dans le titre, juste après le préfixe de type. Pour la Côte d'Ivoire, le pays
est encodé sans accent ni apostrophe, sur DEUX segments :
    « aoi-cote-divoire-... », « ami-cote-divoire-... » -> Côte d'Ivoire (conservé)
    « aao-senegal-... », « spn-mali-... »              -> autre pays (rejeté)
On parcourt donc les pages de listing et on ne conserve QUE les avis dont le
pays extrait vaut explicitement la Côte d'Ivoire. Tout avis rattaché à un autre
pays (ou dont le pays est indéterminé) est rejeté.
...
# Le pays cible est encodé sur deux segments juste après le préfixe de type.
# Variantes possibles : « cote-divoire » (forme observée), « cote-d-ivoire »,
# « ivory-coast ».
_CI_SLUG_RE = re.compile(
    r"^[a-z]{2,6}-(?:cote-d-?ivoire|ivory-coast)-", re.IGNORECASE
)

# Extrait le 1er segment de pays du slug (« aao-mali-… » -> « mali »). Pour un
# avis ivoirien, capture « cote » — d'où le traitement prioritaire par _CI_SLUG_RE.
_SLUG_COUNTRY_RE = re.compile(r"^[a-z]{2,6}-([a-z]+)-", re.IGNORECASE)

_NOTICE_SLUG_RE = re.compile(r"^[a-z]{2,6}-[a-z]", re.IGNORECASE)

# Pays extrait d'un titre « AOI - Cote dIvoire - … » / « AAO - Côte d'Ivoire - … ».
_TITLE_COUNTRY_RE = re.compile(
    r"^\s*[A-Za-zÉ]{2,6}\s*[-–]\s*([A-Za-zÀ-ÿ'’ ]+?)\s*[-–]"
)

CI_TOKENS = ("côte d'ivoire", "cote d'ivoire", "cote divoire",
...
class BadCiScraper(HtmlScraper):
    ...
    def _notice_country_is_target(self, href: str, title: str) -> bool:
        """Ne valide QUE les avis explicitement rattachés à la Côte d'Ivoire."""
        slug = self._slug_of(href)

        # 1) Slug : le signal le plus fiable (pays sur deux segments).
        if _CI_SLUG_RE.match(slug):
            return True
        m = _SLUG_COUNTRY_RE.match(slug)
        if m:
            # Slug avec un 1er segment de pays. « cote » est déjà capté par
            # _CI_SLUG_RE ci-dessus ; tout autre 1er segment (mali, senegal,
            # togo, ethiopia…) -> rejet.
            return m.group(1).lower() == "cote"

        # 2) Repli sur le titre « TYPE - Pays - … ».
        mt = _TITLE_COUNTRY_RE.match(title or "")
        if mt:
            country = mt.group(1).strip().lower()
            return any(tok in country for tok in CI_TOKENS)
    ...
    def _parse_listing(self, soup) -> list[dict]:
        ...
        for a in soup.select("a[href*='/documents/']"):
            ...
            items.append(self.make_item(
                ...
                source_url=full,
                dao_url=full,
                external_id=f"bad-ci-{abs(hash(full)) % (10**10)}",
                tender_type="prive",
            ))
        return items
    ...
    def collect(self) -> list[dict]:
        ...
        for seg in LISTING_SEGMENTS:
            for page in range(MAX_PAGES):
                url = (f"{LISTING_BASE}/{seg}"
                       f"?f%5B0%5D=country%3ACote%20dIvoire&page={page}")
                html = self.fetch_html(url)
                if not html:
                    blocked += 1
                    break
                if self._is_blocked(html):
                    logger.warning("[CI] BAD — page bloquée (Cloudflare) : %s",
                                   url)
                    blocked += 1
                    break
                pages_read += 1
        ...
        if not items and blocked:
            logger.warning("[CI] BAD — aucune donnée collectée (%s pages "
                           "inaccessibles). Portail probablement protégé.",
                           blocked)
        else:
            logger.info("[CI] BAD — %s avis Côte d'Ivoire retenus (%s pages "
                        "lues, %s pages bloquées/injoignables).",
