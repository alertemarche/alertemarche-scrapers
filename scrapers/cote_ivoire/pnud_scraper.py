"""Robot de collecte — Côte d'Ivoire 🇨🇮 · PNUD (marchés PRIVÉS / Nations Unies).

Source : portail mondial des avis de marchés du PNUD —
https://procurement-notices.undp.org/  (filtré sur le bureau Côte d'Ivoire,
UNDP-CIV). Chaque avis est un `a.vacanciesTable__row` présentant « Title »,
...
logger = logging.getLogger("scrapers.cote_ivoire.pnud")

from common.html_base import HtmlScraper  # noqa: E402


class PnudCiScraper(HtmlScraper):
    country = "CI"
    source_name = "PNUD Côte d'Ivoire — Programme des Nations Unies pour le Développement"
    tender_type = "prive"
    ...
    def collect(self) -> list[dict]:
        ...
        if not soup:
            logger.warning("[CI] PNUD injoignable — 0 item")
            return items
        ...
        for row in rows:
            ...
            up = text.upper()
            # Filtre Côte d'Ivoire : bureau UNDP-CIV ou pays.
            if "UNDP-CIV" not in up and "/COTE D'IVOIRE" not in up and "IVOIRE" not in up:
                continue
            ...
            external_id = None
            if reference:
                external_id = "pnud-ci-" + re.sub(r"[^a-zA-Z0-9]", "", reference)[:40]
            else:
                mid = re.search(r"(?:nego_id|notice_id)=(\d+)", source_url)
                if mid:
                    external_id = f"pnud-ci-{mid.group(1)}"
        ...
        logger.info("[CI] PNUD : %d avis collectés", len(items))
        return items


def build() -> PnudCiScraper:
    return PnudCiScraper()
