"""Robot de collecte — Togo 🇹🇬 · PNUD (marchés PRIVÉS / Nations Unies).

Source : portail mondial des avis de marchés du PNUD —
https://procurement-notices.undp.org/  (filtré sur le bureau Togo, UNDP-TGO).
...
logger = logging.getLogger("scrapers.togo.pnud")

from common.html_base import HtmlScraper  # noqa: E402


class PnudTgScraper(HtmlScraper):
    country = "TG"
    source_name = "PNUD Togo — Programme des Nations Unies pour le Développement"
    tender_type = "prive"
    ...
    def collect(self) -> list[dict]:
        ...
        if not soup:
            logger.warning("[TG] PNUD injoignable — 0 item")
            return items
        ...
        for row in rows:
            ...
            up = text.upper()
            # Filtre Togo : bureau UNDP-TGO ou pays /TOGO.
            if "UNDP-TGO" not in up and "/TOGO" not in up and "TOGO" not in up:
                continue
            ...
            external_id = None
            if reference:
                external_id = "pnud-tg-" + re.sub(r"[^a-zA-Z0-9]", "", reference)[:40]
            else:
                mid = re.search(r"(?:nego_id|notice_id)=(\d+)", source_url)
                if mid:
                    external_id = f"pnud-tg-{mid.group(1)}"
        ...
        logger.info("[TG] PNUD : %d avis collectés", len(items))
        return items


def build() -> PnudTgScraper:
    return PnudTgScraper()
