"""Robot de collecte — Sénégal 🇸🇳 · BAD (Banque Africaine de Développement).

Source : portail des avis de passation de marchés de la BAD (www.afdb.org),
filtré sur le Sénégal. On ne conserve que les avis liés au Sénégal.
...
tente plusieurs pages candidates, filtre sur le Sénégal et — en cas de blocage —
...
CANDIDATE_URLS = [
    "https://www.afdb.org/en/documents/project-related-procurement/procurement-notices/specific-procurement-notices?f%5B0%5D=country%3ASenegal",
    "https://www.afdb.org/en/documents/project-related-procurement/procurement-notices/request-for-expression-of-interest?f%5B0%5D=country%3ASenegal",
    "https://www.afdb.org/en/documents/project-related-procurement/procurement-notices/invitation-for-bids?f%5B0%5D=country%3ASenegal",
]
...
class BadSnScraper(HtmlScraper):
    ...
    def _parse_listing(self, soup) -> list[dict]:
        ...
        for row in soup.select("div.views-row, article, tr"):
            ...
            if len(text) < 12 or not self._is_sn(text):
                continue
            ...
            items.append(self.make_item(
                ...
                source_url=href,
                dao_url=href,
                external_id=f"bad-sn-{abs(hash(href)) % (10**10)}",
                tender_type="prive",
            ))
        return items
    ...
    def collect(self) -> list[dict]:
        ...
        for url in CANDIDATE_URLS:
            ...
            if "cloudflare" in low and ("attention required" in low or "verify" in low):
                logger.warning("[SN] BAD — page bloquée par Cloudflare : %s", url)
                blocked += 1
                continue
        ...
        if not items and blocked:
            logger.warning("[SN] BAD — aucune donnée (%s/%s pages inaccessibles).",
                           blocked, len(CANDIDATE_URLS))
        logger.info("[SN] BAD : %d avis collectés", len(items))
        return items
