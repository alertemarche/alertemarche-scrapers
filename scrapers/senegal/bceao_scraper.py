"""Robot de collecte — Sénégal 🇸🇳 · BCEAO (siège de Dakar).
...
marchés pour l'ensemble de ses sites de l'UEMOA. Son SIÈGE se trouvant à Dakar,
une part importante de ses avis concerne le Sénégal. On ne conserve ici que les
avis rattachés au Sénégal (Dakar / Siège).
...
logger = logging.getLogger("scrapers.senegal.bceao")

LISTING_URL = "https://www.bceao.int/fr/appels-offres/appels-offres-marches-publics-achats"
# Un avis concerne le Sénégal s'il vise Dakar, le Sénégal, ou le Siège de la BCEAO.
SN_TOKENS = ("dakar", "senegal", "sénégal", "sénégalais", "siege", "siège")


class BceaoSnScraper(HtmlScraper):
    country = "SN"
    source_name = "BCEAO — Banque Centrale des États de l'Afrique de l'Ouest (Dakar)"
    tender_type = "prive"
    method = "html"
    ...
    def collect(self) -> list[dict]:
        ...
        if not soup:
            logger.warning("[SN] BCEAO injoignable — 0 item")
            return items
        ...
        for row in soup.select("div.views-row"):
            ...
            # Filtrer : ne garder que les avis liés au Sénégal (Dakar / Siège).
            if not self._is_sn(title) and not self._is_sn(full_text):
                continue
            ...
            items.append(self.make_item(
                ...
                source_url=href,
                dao_url=href,
                external_id=f"bceao-sn-{abs(hash(href)) % (10 ** 10)}",
                tender_type="prive",
            ))

        logger.info("[SN] BCEAO (Dakar) : %d avis collectés", len(items))
        return items


def build() -> BceaoSnScraper:
    return BceaoSnScraper()
