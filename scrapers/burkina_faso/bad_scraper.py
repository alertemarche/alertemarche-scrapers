"""Robot de collecte — Burkina Faso 🇧🇫 · BAD (Banque Africaine de Développement / AfDB).

Source : portail des avis de passation de marchés de la BAD (www.afdb.org).

⚠️ Le portail afdb.org n'est PAS filtrable côté serveur : la page renvoie TOUS
les avis, tous pays confondus. Chaque avis encode toutefois son pays de façon
fiable dans l'URL (slug) ET dans le titre, juste après le préfixe de type :
    « aoi-burkina-faso-... », « ami-burkina-faso-... » -> Burkina Faso (conservé)
    « aao-cote-divoire-... », « spn-mali-... »          -> autre pays (rejeté)
On ne conserve donc QUE les avis explicitement rattachés au Burkina Faso.

Le portail est protégé par un pare-feu applicatif (Cloudflare) ; le scraper reste
DÉFENSIF : en cas de blocage il journalise un avertissement et retourne une liste
...
logger = logging.getLogger("scrapers.burkina_faso.bad")
...
# Le pays cible est le token « burkina » juste après le préfixe de type.
_BF_SLUG_RE = re.compile(r"^[a-z]{2,6}-burkina", re.IGNORECASE)
...
class BadBfScraper(HtmlScraper):
    country = "BF"
    ...
    def _notice_country_is_target(self, href: str, title: str) -> bool:
        """Ne valide QUE les avis explicitement rattachés au Burkina Faso."""
        slug = self._slug_of(href)

        if _BF_SLUG_RE.match(slug):
            return True
        m = _SLUG_COUNTRY_RE.match(slug)
        if m:
            return m.group(1).lower() in ("burkina",)

        mt = _TITLE_COUNTRY_RE.match(title or "")
        if mt:
            country = mt.group(1).strip().lower()
            return any(country.startswith(tok) for tok in BF_TOKENS)
    ...
    def _parse_listing(self, soup) -> list[dict]:
        ...
        for a in soup.select("a[href*='/documents/']"):
            ...
            items.append(self.make_item(
                ...
                source_url=full,
                dao_url=full,
                external_id=f"bad-bf-{abs(hash(full)) % (10**10)}",
                tender_type="prive",
            ))
        return items
    ...
    def collect(self) -> list[dict]:
        ...
        for seg in LISTING_SEGMENTS:
            for page in range(MAX_PAGES):
                url = (f"{LISTING_BASE}/{seg}"
                       f"?f%5B0%5D=country%3ABurkina+Faso&page={page}")
                html = self.fetch_html(url)
                if not html:
                    blocked += 1
                    break
                if self._is_blocked(html):
                    logger.warning("[BF] BAD — page bloquée (Cloudflare) : %s", url)
                    blocked += 1
                    break
                pages_read += 1
        ...
        if not items and blocked:
            logger.warning("[BF] BAD — aucune donnée collectée (%s pages "
                           "inaccessibles). Portail probablement protégé.", blocked)
        else:
            logger.info("[BF] BAD — %s avis Burkina Faso retenus (%s pages lues, "
                        "%s pages bloquées/injoignables).",
                        len(items), pages_read, blocked)
        return items


def build() -> BadBfScraper:
    return BadBfScraper()
